// 用 tasks.js 重灌两张实习生表（先清空再写入）。用法：node reseed.js [Ryan|Pham]
...
(async () => {
  for (const it of T.INTERNS) {
    ...
    const records = T.TASKS.map((t) => ({ fields: {
      ...
      '登记到哪 Output': t.out.replace('Ryan 工作群', it.group), '手册章节 Manual §': t.sec, '来源 Source': t.src, '序号 #': t.n, '工作日期 Date': today,
    } }));
