...
| **在 Windows 常开机上值守的同事** | [`roi-monitor-win/`](roi-monitor-win/) | 双击 `.bat` 就能跑的预警程序，结果落桌面；不放心就先双击「暴力测试」，不用登录也不碰真实数据 |
...
| [`roi-monitor/`](roi-monitor/) | GMV Max 高成本低 ROI 素材预警：每小时只读扫描 TikTok 广告后台，命中写 xlsx / 飞书多维表格并推送；每半小时一次「运营播报」（当天各计划成本 / 成交 / ROI + 近 1 小时增量）；可自动发现新建计划、把运行日志回传飞书 | Node.js · Playwright | Mac launchd 常驻 |
| [`roi-monitor-win/`](roi-monitor-win/) | 同一套扫描内核的 Windows 精简版，放在 24h 常开的电脑上跑，一堆 `.bat` 双击即用（含播报、发现计划、暴力测试） | Node.js · Playwright | Windows 常驻 |
...
TikTok 广告后台 ──(roi-monitor 每小时只读扫描)──▶ 飞书多维表格「命中明细」
              └─(每半小时读计划列表)──▶ 飞书群「运营播报」卡片 + 多维表格「运行日志」
                                                        │
